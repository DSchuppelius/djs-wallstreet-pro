    ...
    echo $service_icon_image;
       ...
       if (has_post_thumbnail()) { ?> 
					<div class="service-box">
							...
							<a href="<?php echo $meta_service_link; ?>" <?php if (get_post_meta(get_the_ID(), "meta_service_target", true)) {
    echo 'target="_blank"';
} ?>> <?php the_post_thumbnail("theme_service_img", $defalt_arg); ?> </a>
						<?php } else { ?>
			...
			<?php if (get_the_title()) { ?>		
				<h2><a href="<?php echo $meta_service_link; ?>" <?php if (get_post_meta(get_the_ID(), "meta_service_target", true)) {
    echo "target='_blank'";
} ?> > <?php echo the_title(); ?> </a></h2><?php } ?>
				<?php if (!empty(get_post_meta(get_the_ID(), "service_description_text", true))): ?><p><?php echo $excerpt = get_post_meta(get_the_ID(), "service_description_text", true); ?></p><?php endif; ?>
				<?php if (get_post_meta(get_the_ID(), "service_readmore_text", true)): ?><div class="service-btn"><a href="<?php echo $meta_service_link; ?>" <?php if (get_post_meta(get_the_ID(), "meta_service_target", true)) {
    echo "target='_blank'";
} ?> ><?php echo get_post_meta(get_the_ID(), "service_readmore_text", true); ?></a></div><?php endif; ?>
			</div>	
		</div>
		<?php

...
<div class="container page about-us team">
    ...
    <div class="row <?php row_frame_border(""); ?> about-section text team-title">
        <div class="section_heading_title<?php innerrow_frame_border(" "); ?>">
            <?php if ($current_setup_posttypes->get("about_team_title") != "") { ?><h1>
                <?php echo $current_setup_posttypes->get("about_team_title"); ?></h1>
            ...
            <?php } ?>
            <?php if ($current_setup_posttypes->get("about_team_description") != "") { ?><p>
                <?php echo $current_setup_posttypes->get("about_team_description"); ?></p><?php } ?>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="row <?php row_frame_border(""); ?> about-section text team-section">
        ...
        <div class="col-md-3 col-sm-6 team-effect">
            ...
            <div class="team-area<?php big_border(" "); ?>">
                <h5><?php the_title(); ?><span><?php if (!empty($designation_meta_save)) { echo $designation_meta_save; } ?></span>
                </h5>
                <div class="desi-seperate"></div>
                <p><?php if (!empty($description_meta_save)) { echo $description_meta_save; } ?></p>
                <ul class="team-social-icons">
                    <?php if ($fb_meta_save) { ?>
                    <li><a href="<?php if ($fb_meta_save) { echo esc_html($fb_meta_save); } ?>"
                            <?php blank_target($fb_meta_save_chkbx); ?>><i class="fa-brands fa-facebook"></i></a></li>
                    <?php } ?>
                    <?php if ($skype_meta_save) { ?>
                    <li><a href="<?php if ($skype_meta_save) { echo esc_html($skype_meta_save); } ?>"
                            <?php blank_target($skype_meta_save_chkbx); ?>><i class="fa-brands fa-skype"></i></a></li>
                    <?php } ?>
                    <?php if ($rss_meta_save) { ?>
                    <li><a href="<?php if ($rss_meta_save) { echo esc_html($rss_meta_save); } ?>"
                            <?php blank_target($rss_meta_save_chkbx); ?>><i class="fa fa-rss"></i></a></li>
                    <?php } ?>
                    <?php if ($lnkd_meta_save) { ?>
                    <li><a href="<?php if ($lnkd_meta_save) { echo esc_html($lnkd_meta_save); } ?>"
                            <?php blank_target($lnkd_meta_save_chkbx); ?>><i class="fa-brands fa-linkedin"></i></a></li>
                    <?php } ?>
                    <?php if ($twt_meta_save) { ?>
                    <li><a href="<?php if ($twt_meta_save) { echo esc_html($twt_meta_save); } ?>"
                            <?php blank_target($twt_meta_save_chkbx); ?>><i class="fa-brands fa-twitter"></i></a></li>
                    <?php } ?>
                </ul>
            </div>
        </div>
        ...
        <div class="col-md-3 col-sm-6 team-effect">
            <div class="team-box">
                <img class="img-responsive"
                    src="<?php echo THEME_ASSETS_PATH_URI; ?>/images/team/team<?php echo $i; ?>.jpg">
            </div>
            <div class="team-area<?php innerrow_frame_border(" ");?>">
                <h5><?php echo $team_title[$i - 1]; ?> <span><?php echo $team_designation[$i - 1]; ?></span></h5>
                <div class="desi-seperate"></div>
                <p><?php echo "Lorem ipsum dolor sit amet, conet adipis cing. Lorem ipsum dolore sit amet, consectetur adipisicings elit"; ?>
                </p>
                <ul class="team-social-icons">

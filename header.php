...
<head>
    ...
    if (!empty($wp_query->post)) {
        ...
        if (!empty($tags))
            foreach ($tags as $tag) {
                $single_metakeytags .= ", " . wp_strip_all_tags($tag->name);
            }
    } ?>
    ...
    <meta name="description"
        content="<?php if (is_single()) {
                        echo esc_attr(wp_trim_words($single_description, 120, '...'));
                    } else if (!empty($page_description)) {
                        echo esc_attr(get_bloginfo("name") . " - " . $page_description);
                    } else {
                        echo esc_attr(get_bloginfo("name") . " - " . get_bloginfo("description"));
                    } ?>" />
    <meta name="keywords" content="<?php echo esc_attr($single_metakeytags); ?>" />
...
<body id="djs-body" <?php body_class($additional_BodyClasses); ?>
    <?php if (!$current_setup->get("contextmenu_enabled")) {
        echo 'oncontextmenu="return false;"';
    } ?>>
    <?php wp_body_open(); ?>
    <?php if ($current_setup->get("page_fader_enabled")) {
        get_template_part("template-parts/global/header/page", "fader");
    } ?>
    <div id="wall_wrapper">

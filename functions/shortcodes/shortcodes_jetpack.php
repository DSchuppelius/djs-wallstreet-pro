...
function jetpack_bruteforce_counter($atts, $content=null) {
    ...
    if(is_plugin_active('jetpack/jetpack.php')) {
        if (Jetpack::is_module_active('protect')) {
            $result .= esc_html(number_format_i18n(get_site_option('jetpack_protect_blocked_attempts', 0))) . " " . __("visits", "wallstreet");
        } else {
            $result .= __("not enabled", "wallstreet");
        }
    } else {
        $result .= __("nonexistent", "wallstreet");
    }
...
function akismet_spam_counter($atts, $content=null) {
    ...
    if(is_plugin_active('akismet/akismet.php')) {
        $result .= esc_html(number_format_i18n(get_site_option('akismet_spam_count', 0))) . " " . __("messages", "wallstreet");
    } else {
        $result .= __("nonexistent", "wallstreet");
    }
